/**
 * Display label maps for all API enum slugs.
 * Keys must stay in sync with `server/src/models/enums.ts`
 * (guarded by enums.test.ts for the opportunity-facing groups).
 */

export type EnumMap = Readonly<Record<string, string>>

export const EKEHI_ENUMS = {
  opportunityType: {
...
} as const satisfies Record<string, EnumMap>
...
export function enumLabel(enumMap: EnumMap, value: string): string | undefined {
