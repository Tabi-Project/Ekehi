  ...
  return (
    <ModalPortal>
      ...
      <DialogPrimitive.Content
        className={cn(
          'border-line bg-surface fixed top-1/2 left-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border p-6 shadow-xl',
          className,
        )}
        {...props}
      >
        ...
        {hideClose ? null : (
          <DialogPrimitive.Close
            aria-label="Close"
            className="text-content-muted hover:text-content focus-visible:ring-primary absolute top-4 right-4 rounded p-1 focus-visible:ring-2 focus-visible:outline-none"
          >
...
function ModalTitle({
  ...
  return (
    <DialogPrimitive.Title
      className={cn('text-content text-xl font-semibold', className)}
      {...props}
    />
  )
}
