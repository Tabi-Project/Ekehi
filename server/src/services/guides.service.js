...
const getGuides = async ({ search, category, page = 1, limit = 10 }) => {
  const offset = (page - 1) * limit;

  let query = supabase
    .from("guides")
    .select("id, title, slug, summary, category, created_at", { count: "exact" })
...
const createGuide = async (submittedBy, fields) => {
  const { data, error } = await supabase
    .from("guides")
...
const updateGuide = async (id, submittedBy, fields) => {
  ...
  if (!existing) throw Object.assign(new Error("Guide not found"), { status: 404 });
